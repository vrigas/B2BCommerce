...
import setTokenizedCard from '@salesforce/apex/B2BPaymentController.setTokenizedCard';
...
export default class PaymentMethod extends LightningElement {
  ...
  async handlePaymentButton() {
    ...
    if (
      ...
      const card = {
        'card[name]': creditCardData.cardHolderName,
      ...
      try {
        const resp = await fetch('https://api.stripe.com/v1/tokens', {
          ...
          headers: {
            ...
            'Authorization': 'Bearer pk_test_51NJygcAjyCDWBtdL8ns8T7MrB872hGIaaF9NqIv49Ww4u3POITVjGKGahqcNGDazL5zeNgy3kbIMdl38ajdmla6p00W2hXsBGt',
            'Host': 'api.stripe.com'
          },
          body: formBody
        })
        ...
        await setTokenizedCard({ cartId: this.cartId, tokenizeInfo: responseData })
      } catch (err) {
        alert(err)
        return
      }
